...
//checking if request cotains multipart data
            ...
            if (isMultipart) {
                ...
                while (itr.hasNext())//checking if there is any value in the position like rs.next()
                {
                    ...
                    } else//when it is an image or multipart
                    {
                        ...
                        if (field_name.equals("file_photo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\Photo\\";//path for storing image
                        ...
                        if (field_name.equals("file_proof"))//write the name of image here it is img_product
                        {
                            ...
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\Proof\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            fn1 = "proof_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
                            savedFile = new File(file_path + fn);//fn contains name of file
                            try {
                                //writing the file object
                ...
                String ins = "insert into tbl_user(user_name,user_dob,user_gender,user_email_1,user_email_2,user_contact_1,user_contact_2,user_photo,user_proof,user_address,place_id,user_pasword,user_doj)"
                        + "values('" + value[0] + " " + value[1] + "','" + value[2] + "','" + value[3] + "','" + value[4] + "','" + value[5] + "','" + value[6] + "','" + value[7] + "','" + fn + "','" + fn1 + "','" + value[8] + "','" + value[12] + "','" + value[14] + "',curdate())";
        ...
        %> 
                <script type="text/javascript">
                    ...
                    setTimeout(function() {
                        window.location.href = '../../Registartion.jsp'
